...
from .excel_open_service import open_excel_location, resolve_quotation_file
...
OPENMOON_AGENT_TOOLS: list[dict[str, Any]] = [
    ...
    {
        "type": "function",
        "name": "open_quotation_source",
        "description": "사용자가 검색된 과거 견적 Excel을 열어달라고 명시적으로 요청할 때 호출합니다. 반드시 search_quotation_history 결과의 source_file과 source_sheet를 그대로 사용합니다.",
        "strict": True,
        "parameters": {
            "type": "object",
            "properties": {
            ...
            "required": ["source_file", "source_sheet"],
            "additionalProperties": False,
        },
    },
    {
    ...
    },
    {
        "type": "function",
        "name": "add_quote_item",
        "description": "사용자가 새 견적 품목을 추가해 달라고 명확히 요청할 때 품목 한 개를 추가합니다. 사용자가 말하지 않은 값은 null로 둡니다.",
        "strict": True,
        "parameters": {
            "type": "object",
            "properties": {
    ...
    {
        "type": "function",
        "name": "replace_quote_items",
        "description": "사용자가 여러 품목을 자연어로 새로 작성하고 현재 품목 전체를 그 내용으로 교체해 달라고 요청할 때 사용합니다. 기존 품목을 모두 대체하므로 명확한 요청에만 호출합니다.",
        "strict": True,
        "parameters": {
            "type": "object",
            "properties": {
    ...
    },
    {
        ...
        "description": "사용자가 특정 견적 품목을 삭제해 달라고 명확히 요청할 때 사용합니다.",
        "strict": True,
        "parameters": {
            "type": "object",
            "properties": {
            ...
            "required": ["item_position", "product_name"],
            "additionalProperties": False,
        },
    },
    {
...
def _search_quotation_history(
    ...
    *,
    ...
    limit = max(1, min(limit, 20))
    rows = get_external_history_candidates(
    ...
    if customer_name:
        ...
        rows = [
            ...
            if query in " ".join(
                ...
                for field in ("customer_name", "customer_organization")
            ).casefold()
        ]
    if product_name:
        ...
        rows = [
            ...
            if query in " ".join(
                str(row.get(field) or "")
                for field in ("product_name", "specification")
            ).casefold()
        ]
    rows = rows[:limit]
    ...
    if not known:
        known = ctx.session.scalar(
            select(QuotationHistory.id).where(
                QuotationHistory.source_file == source_file,
                QuotationHistory.source_sheet == source_sheet,
            )
        ) is not None
    if not known:
        raise PermissionError("검색된 과거 견적 DB에 등록된 파일만 열 수 있습니다.")
    resolved_source = resolve_quotation_file(
        source_file,
...
def _new_mail_item(ctx: AgentToolContext, data: dict[str, Any], position: int) -> MailItem:
    ...
    if not product_name:
        raise ValueError("품목명은 비워둘 수 없습니다.")
    unit_price_raw = data.get("unit_price")
    if unit_price_raw is not None and ctx.user_message_id is not None:
        message = ctx.session.get(ChatMessage, ctx.user_message_id)
        price = int(round(float(unit_price_raw)))
        text = (message.content if message else "").replace(",", "").replace(" ", "")
        explicit = str(price) in text
        if price % 10000 == 0:
            explicit = explicit or f"{price // 10000}만" in text
        if price % 1000 == 0:
            explicit = explicit or f"{price // 1000}천" in text
        if not explicit:
            unit_price_raw = None
    unit_price = int(round(float(unit_price_raw))) if unit_price_raw is not None else None
    ...
    return MailItem(
        ...
        amount=calculate_supply_amount(quantity, unit_price),
        schedule_note=data.get("schedule_note"),
        evidence=evidence,
        confirmed=unit_price is not None,
    )


def _add_quote_item(ctx: AgentToolContext, **arguments: Any) -> dict[str, Any]:
    item = _new_mail_item(ctx, arguments, len(ctx.mail.items) + 1)
...
def _replace_quote_items(ctx: AgentToolContext, *, items: list[dict[str, Any]]) -> dict[str, Any]:
    if not items:
        raise ValueError("교체할 품목이 한 개 이상 필요합니다.")
    for old_item in list(ctx.mail.items):
        ctx.session.delete(old_item)
    ctx.mail.items.clear()
    ctx.session.flush()
    for position, data in enumerate(items, start=1):
...
def _delete_quote_item(
    ...
    *,
    ...
    item = _find_item(ctx, item_position=item_position, product_name=product_name)
    deleted = {"item_id": item.id, "product_name": item.product_name, "position": item.position}
    ctx.mail.items.remove(item)
    ctx.session.delete(item)
    for position, remaining in enumerate(ctx.mail.items, start=1):
...
def _create_quotation_draft(ctx: AgentToolContext) -> dict[str, Any]:
    ...
    return {
        "created": False,
        "ready": True,
        "requires_storage_selection": True,
        "message": "견적 내용 검증이 완료되었습니다. 사용자가 저장 방식과 파일을 선택해야 합니다.",
    }
...
def execute_agent_tool(
    ...
    if name == "get_current_quote":
    ...
    elif name == "open_quotation_source":
        result = _open_quotation_source(ctx, **arguments)
        evidence = [{
            "type": "history_excel_opened",
            "label": "과거 견적 Excel 열기",
            "source_file": result.get("file_path"),
            "locations": [{
                "sheet": result.get("sheet") or arguments.get("source_sheet") or "",
                "cell": "",
            }],
        }]
        ...
        result = _add_quote_item(ctx, **arguments)
        evidence = [{"type": "agent_action", "label": f"{result['action']['product_name']} 품목 추가"}]
    elif name == "replace_quote_items":
        result = _replace_quote_items(ctx, **arguments)
        evidence = [{"type": "agent_action", "label": f"견적 품목 {result['action']['count']}개로 교체"}]
    elif name == "delete_quote_item":
        result = _delete_quote_item(ctx, **arguments)
        evidence = [{"type": "agent_action", "label": f"{result['action']['product_name']} 품목 삭제"}]
