...
def resolve_quotation_file(file_path: Path | str, quotation_root: Path) -> Path:
    """Resolve stale history DB paths against the portable quotation_files root."""
    original = Path(file_path).expanduser()
    if original.exists() and original.is_file():
        return original.resolve()

    root = quotation_root.expanduser().resolve()
    if not root.exists():
        raise FileNotFoundError(f"quotation_files 폴더를 찾을 수 없습니다: {root}")

    matches = sorted(
        (path.resolve() for path in root.rglob(original.name) if path.is_file()),
        key=lambda path: (len(path.relative_to(root).parts), str(path).casefold()),
    )
    if not matches:
        raise FileNotFoundError(
            f"quotation_files에서 견적 파일을 찾을 수 없습니다: {original.name}"
        )
    return matches[0]
