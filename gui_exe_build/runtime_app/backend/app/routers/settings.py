...
def status(settings: Settings = Depends(get_settings)):
    return HealthOut(
        ...
        live_send_enabled=settings.allow_live_send or settings.approval_test_mode,
    )
...
def paths(settings: Settings = Depends(get_settings)):
    return {
        ...
        "approval_test_mode": str(settings.approval_test_mode),
        "approval_test_recipient": settings.approval_test_recipient,
