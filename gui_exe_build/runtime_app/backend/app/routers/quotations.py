...
from ..services.excel_open_service import resolve_quotation_file
...
def email_preview(
    ...
    return EmailPreview(
        ...
        recipient=(
            settings.approval_test_recipient
...
def delete_draft(
    ...
    path: Path | None = Path(draft.file_path).resolve()
    ...
    try:
        ...
        try:
            ...
            # 과거 DB의 절대경로가 남아 있으면 portable quotation_files에서
            # 같은 파일을 다시 찾는다. 못 찾더라도 외부 파일은 건드리지 않고
            # 초안 기록만 안전하게 삭제한다.
            try:
                path = resolve_quotation_file(path, quotation_root)
                removable = path.name.startswith("견적서_")
    ...
    if removable and path is not None and path.exists():
        path.unlink()
